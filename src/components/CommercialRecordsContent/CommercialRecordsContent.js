...
import "./CommercialRecordsContent.scss";
import LicenceCard from "../LicenceCard/LicenceCard";

const CommercialRecordsContent = () => {
  const commercialRecordsJson = require("../../assets/json/commercialRecords.json");
  const InsuranceLicencesJson = require("../../assets/json/insuranceLicences.json");
  const commercialTabs = require("../../assets/json/commercialTabs.json");

  const [active, setActive] = useState(1);
  let [records, updateRecords] = useState(commercialRecordsJson.records);
  let licences = InsuranceLicencesJson.licences;

  const selectTab = (tab) => {
    setActive(tab.id);
    if (tab.id === 1) {
      updateRecords(commercialRecordsJson.records);
      return;
    }
    let filteredItems = commercialRecordsJson.records.filter((item) => {
      switch (tab.id) {
        case 2:
  ...
  const toggleFavourite = (item) => {
    let updatedRecords = commercialRecordsJson.records.map((record) => {
    ...
    <>
      <section className="records" key={"records"}>
        ...
        <ul className="nav nav-pills px-0 mb-20 ">
          {commercialTabs.tabs.map((tab) => {
            return (
              <li className="nav-item " key={`t-${tab.id}`}>
                <button
                  className={`nav-link records-tabs fw-normal ${
...
export default CommercialRecordsContent;
